...
use crate::blocks::intermediary::data::{ModernBlockList, PropertyValue, ModernBlockData};
...
impl<'a, 'raw, 'de: 'raw> Visitor<'de> for CompactRuleProvider<'a, 'raw> {
            ...
            A: serde::de::MapAccess<'de>,
    {
        ...
        while let Some((identifier, block)) = map.next_entry::<Identifier<'raw>, RawBlockData<'raw>>()? {
            ...
            // make sure there is the correct amound of blockstates
            ...
            for state in &block.states {
                ...
                if state.properties.len() != property_count {
                    return Err(serde::de::Error::invalid_length(
                        state.properties.len(), &format!("{} entries", property_count).as_str(),
                    ));
                }
                ...
                for (&name, &value) in state.properties.iter().rev() {
                    let values = block.properties.iter().find_map(|(&a, b)| if a == name { Some(b) } else { None })
                        .ok_or(serde::de::Error::custom(
                            format!("found a non-matching a property \"{}\" for \"{}\"", name, identifier)
                        ))?;
                    match values.iter().position(|&x| x == value) {
                        ...
                        None => {
                            return Err(serde::de::Error::custom(
                                format!(
                                    "invalid property value \"{}\" found while deserializing \"{}\"",
                                    value,
                                    identifier
                                ),
                            ));
                ...
                if id != state.id {
                    return Err(serde::de::Error::custom(
                        format!("incorrect id match for \"{}\", found {} while expecting {}",
                            identifier,
                            state.id,
                            id
                        ),
            ...
            let default_id = default_id.and_then(|id| if id == base_id {
                None
            } else {
                Some(id)
            });
            ...
            properties.extend(
                self.transform(block.properties())
            ...
            // Collect the property types to add to the compacted form of this block (see below)
            let properties = block.properties()
                .map(|(name, kind)| if name == "type" {
                    ("kind", kind)
                } else {
                    (name, kind)
                })
